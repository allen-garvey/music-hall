defmodule RenameAudioFiles do
  ...
  def exit_with_error do
    IO.puts(
      :stderr,
      "usage: elixir #{Path.basename(__ENV__.file)} <source_directory> <is_dry_run>"
    )
  end
  ...
  def rename_master_files_in_directory_from_compressed_files(directory, is_dry_run)
      when is_boolean(is_dry_run) do
  ...
  def rename_files(directory, is_dry_run) when is_boolean(is_dry_run) do
    compressed_files =
      File.ls!(directory)
      |> Enum.filter(fn file -> Path.extname(file) |> String.match?(~r/\.(mp3|m4a)$/) end)

    source_files =
      File.ls!(directory)
      |> Enum.filter(fn file -> Path.extname(file) |> String.match?(~r/\.(wav|aif)$/) end)

    for source_file <- source_files do
      {best_match, distance} =
        compressed_files
        |> Enum.map(fn compressed_file ->
          {compressed_file,
           String.bag_distance(Path.rootname(source_file), Path.rootname(compressed_file))}
        end)
        |> Enum.reduce({"", 0}, fn {current_compressed_file, current_distance},
                                   {acc_file, acc_distance} ->
...
case System.argv() do
  [source_dir] ->
    RenameAudioFiles.rename_master_files_in_directory_from_compressed_files(source_dir, false)

  [source_dir, _is_dry_run] ->
    RenameAudioFiles.rename_master_files_in_directory_from_compressed_files(source_dir, true)

  _ ->
    RenameAudioFiles.exit_with_error()
end
